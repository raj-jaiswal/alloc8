                ...
                <div className="w-full max-w-4xl p-5 bg-white rounded-lg shadow-md mt-5">
                    {rooms.map((room, index) => (
                        <div key={index} className="mb-6">
                            <h3 className="text-2xl font-semibold text-gray-800 mb-3">Room {room.roomNumber}</h3>
                            <ul className="space-y-2">
                                {room.roommates.map((roommate, idx) => (
                                    <li key={idx} className="flex justify-between items-center p-3 bg-slate-50 rounded-md shadow-sm">
                                        <span className="text-lg font-medium text-gray-700">{roommate.name}</span>
                                        <span className="text-lg font-medium text-gray-500">{roommate.rollNumber}</span>
                                    </li>
                                ))}
                            </ul>
                        </div>
